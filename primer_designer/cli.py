...
def parse_mutations(mutation_strings: List[str]) -> List[Tuple[str, int, str]]:
    ...
    for mutation_str in mutation_strings:
        ...
        if pos_start == pos_end:
            print(
                f"Error: Could not parse position from mutation: {mutation_str}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Design primers for site-directed mutagenesis"
    )
    parser.add_argument("sequence", help="DNA sequence to use for primer design")
    parser.add_argument(
        "mutations",
        nargs="+",
        help="Mutations in format F10A (original AA, position, new AA)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="Minimum nucleotides flanking each side of mutation (default: 15)",
    )
    parser.add_argument(
        "--max-flank-length",
        type=int,
        default=18,
        help="Maximum nucleotides flanking each side of mutation (default: 18)",
    )
    parser.add_argument(
        "--min-tm",
        type=float,
        default=65.0,
        help="Minimum acceptable melting temperature in °C (default: 65.0)",
    )
    parser.add_argument(
        "--max-tm",
        type=float,
        default=75.0,
        help="Maximum acceptable melting temperature in °C (default: 75.0)",
    )
    parser.add_argument(
        "--optimal-tm",
        type=float,
        default=70.0,
        help="Target optimal melting temperature in °C (default: 70.0)",
    )
    parser.add_argument(
        "--max-tm-diff",
        type=float,
        default=5.0,
        help="Maximum allowed difference between forward/reverse Tm (default: 5.0)",
    )
    parser.add_argument(
        "--min-gc",
        type=float,
        default=35.0,
        help="Minimum acceptable GC content in percent (default: 35.0)",
    )
    parser.add_argument(
        "--max-gc",
        type=float,
        default=70.0,
        help="Maximum acceptable GC content in percent (default: 70.0)",
    )
    parser.add_argument(
        "--optimal-gc",
        type=float,
        default=50.0,
        help="Target optimal GC content in percent (default: 50.0)",
    )
    ...
    if args.json:
        ...
        for result in results:
            if "error" in result:
                ...
                print(
                    f"Tm: {result['forward_tm']}°C, GC: {result['forward_gc'] * 100}%"
                )
                print(f"Reverse primer ({len(result['reverse_primer'])} bp):")
                print(f"5'-{result['reverse_primer']}-3'")
                print(
                    f"Tm: {result['reverse_tm']}°C, GC: {result['reverse_gc'] * 100}%"
                )
